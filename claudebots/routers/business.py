...
_HELP_TEXT = """🤖 *Возможности бота*

*Автоответчик*
Автоматически отвечает на сообщения контактов через бизнес-аккаунт. Каждый контакт получает свой топик в личном чате — туда дублируются все входящие и ответы бота.

*Показания счётчиков*
Начни сообщение с `Показания` и напиши данные в свободной форме:
`Показания: газ 5678, вода 234, день 12540 ночь 4310`
Бот запишет значения в Google Таблицу (листы Gas / Water / Electricity) с автоматическим расчётом расхода и итоговой суммы.

*Панельное обсуждение*
Запускает дискуссию между AI-экспертами (аналитик, скептик, креатив, прагматик, модератор):
`/panel Стоит ли запускать новый продукт?`
`панель: Как масштабировать продажи?`
Или перешли в бота любой пост из канала — обсуждение запустится автоматически.

*Google Календарь*
Бот знает твоё расписание на 10 дней вперёд. Когда контакт предлагает встречу — бот создаёт событие в календаре и уведомляет тебя.

*Перенос прайса*
Когда контакт присылает ссылку на Google Таблицу с ценами — бот читает её, добавляет наценку и переносит позиции в твою личную таблицу.

*Obsidian*
Вся переписка с контактами автоматически сохраняется в заметки:
`Contacts/{имя}.md` — история с конкретным контактом
`Daily/{дата}.md` — дневной лог всех контактов

*Дайджест*
Каждый день в 20:00 бот присылает сводку: кто писал, сколько сообщений, последняя фраза.

*Помощник владельца*
В личном чате с ботом — просто пиши любой вопрос или задачу, бот ответит как персональный AI-ассистент с учётом истории контактов и расписания."""
