...
# Per-platform-key creation locks — prevent concurrent downloads creating duplicate named topics
_media_topic_locks: dict[str, asyncio.Lock] = {}
...
async def _prepare_media_send(
    *,
    ...
    if is_supergroup:
        ...
        if topic_key not in _media_topic_locks:
            _media_topic_locks[topic_key] = _aio.Lock()
        async with _media_topic_locks[topic_key]:
...
💬 *Личный AI-ассистент*
━━━━━━━━━━━━━━━━━━━━━
Пиши боту напрямую — вопрос, задача, анализ. Отвечает с учётом истории переписки с контактами и твоего расписания.
`Что писал Иван на этой неделе?`
`Есть ли у меня окно в пятницу?`

━━━━━━━━━━━━━━━━━━━━━
🎙 *Голосовые сообщения*
━━━━━━━━━━━━━━━━━━━━━
Запиши голосовое — бот транскрибирует (Groq Whisper) и ответит как на текст. Работает в личке и в топиках.

━━━━━━━━━━━━━━━━━━━━━
📄 *PDF — краткое резюме*
━━━━━━━━━━━━━━━━━━━━━
Пришли PDF — бот извлечёт текст и вернёт резюме: главная идея, ключевые пункты, вывод.

━━━━━━━━━━━━━━━━━━━━━
💸 *Учёт расходов*
━━━━━━━━━━━━━━━━━━━━━
Напиши про трату обычными словами — бот распознает сумму и запишет в Google Таблицу.
`Потратил 500 на бензин` · `Купил кофе за 80`
Ключевые слова: потратил, заплатил, купил, стоит, расход, трата.

━━━━━━━━━━━━━━━━━━━━━
📸 *Instagram* · 🎬 *TikTok* · 🐦 *X / Twitter*
━━━━━━━━━━━━━━━━━━━━━
Кинь ссылку на публичный пост/Reel/видео — бот скачает медиа в отдельный топик (создаётся автоматически).
`instagram.com/reel/ABC123/`
`tiktok.com/@user/video/123`
`x.com/user/status/123`

━━━━━━━━━━━━━━━━━━━━━
🎵 *YouTube — аудио и резюме*
━━━━━━━━━━━━━━━━━━━━━
Ссылка на видео → бот пришлёт аудио в топик *🎵 YouTube* (>50 МБ — документом).
`youtu.be/dQw4w9WgXcQ`
Для текстового резюме добавь перед ссылкой `резюме`, `кратко` или `summary`.
Shorts и плейлисты не поддерживаются.

━━━━━━━━━━━━━━━━━━━━━
📊 *Показания счётчиков*
━━━━━━━━━━━━━━━━━━━━━
Начни со слова *Показания* — данные уйдут в Google Таблицу (газ/вода/электр.), расход считается сам.
`Показания: газ 5678, вода 234, день 12540 ночь 4310`
⚠️ Для электричества нужны оба значения: день и ночь.

━━━━━━━━━━━━━━━━━━━━━
🧠 *Панельное обсуждение*
━━━━━━━━━━━━━━━━━━━━━
Дискуссия 5 AI-персон (аналитик, скептик, креатив, прагматик, модератор) + итог с оценкой 👍/👎/🔁.
`/panel Стоит ли запускать продукт?` · `панель: Как масштабировать?`
Или перешли пост из канала — обсудят сами.
Под входящим от контакта есть кнопка *🎯 Спросить панель*.

━━━━━━━━━━━━━━━━━━━━━
⌨️ *Команды*
━━━━━━━━━━━━━━━━━━━━━
*Общие:*
`/help` — справка · `/ping` — бот жив
`/reset` — очистить диалог
`/cost` — расход токенов
`/reload` — перезагрузить персоны
`/contacts` — контакты · `/stats` — статистика

*Панель:*
`/panel <тема>` — запустить
`/panelschedule HH:MM <тема>` — запланировать
`/panelcancel` — отменить план
`/panelstatus` — статус
`/panelbest` · `/panelworst` — лучшие/слабые раунды
`/panelfind <слово>` — поиск по памяти
`/personas` — память персон

*В топике контакта:*
`/mute` (`/pause`) — выключить автоответ
`/unmute` (`/resume`) — включить

━━━━━━━━━━━━━━━━━━━━━
🔄 *Автоматически (фоном)*
━━━━━━━━━━━━━━━━━━━━━
• Утренний брифинг (календарь + сводка)
• Дайджест контактов (кто писал)
• Новостная панель (топ-новости дня)
• Мониторинг каналов → авто-панель по важным постам
• Дайджест канала за сутки
• Напоминания о «забытых» контактах

━━━━━━━━━━━━━━━━━━━━━
🗓 *Календарь* · 📋 *Прайс* · 📝 *Obsidian* · 🔇 *Пауза AI*
━━━━━━━━━━━━━━━━━━━━━
• Бот знает расписание и сам создаёт встречи из переписки.
