    ...
    async def _save_panel_memory(self, key: str, topic: str) -> None:
        ...
        try:
            ...
            memory = await client.complete(
                system=(
                    "Ты сжимаешь итог дискуссии до одного предложения на русском языке. "
                    "Отвечай ТОЛЬКО самим предложением — без пояснений, без мета-комментариев, "
                    "без ссылок на задачу или контекст. Никакого markdown."
                ),
                messages=messages,
                max_tokens=80,
            )
